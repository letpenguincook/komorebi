...
export const FiltrosLaterais: React.FC<FiltrosLateraisProps> = ({
  ...
  return (
    <aside className="space-y-5">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3 shadow-[0_2px_8px_rgba(0,0,0,0.01)]">
        ...
        {gruposDisponiveis.length > 8 && (
          <div className="relative w-full">
            <input
              ...
              className="w-full pl-7 pr-6 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-[11px] focus:outline-none focus:border-[#0284C7]"
            />
            <i className="fa-solid fa-magnifying-glass absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 text-[10px]" />
          </div>
        )}
        ...
        <div className="flex flex-col gap-1">
          <button
            ...
            className={`w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${!grupoId ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'}`}
          >
            Ver todos
          </button>
          ...
          {gruposFiltradosLateral
            ...
            .map((g) => (
              <button
                ...
                className={`w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition-colors truncate ${grupoId && formatarParaUrl(grupoId) === formatarParaUrl(g) ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'}`}
              >
          ...
          {gruposFiltradosLateral.length > LIMITE_VISIVEL && !buscaGrupoLateral && (
            <button
              ...
              className="text-left px-3 py-1 text-[11px] font-bold text-[#0284C7] hover:text-[#0369A1] transition-colors mt-1"
            >
      ...
      {grupoId && subItensDisponiveis.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3 shadow-[0_2px_8px_rgba(0,0,0,0.01)]">
          ...
          <div 
            className={`flex flex-wrap gap-1.5 transition-all overflow-hidden ${
              !expandidoSubItens && subItensDisponiveis.length > 12 ? 'max-h-24' : 'max-h-none'
            }`}
          >
            <button
              onClick={() => navegarLimpo(`/${formatarParaUrl(universoId)}/${formatarParaUrl(grupoId)}`)}
              className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${!subItemId ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}
            >
              Todos
            </button>
            {subItensDisponiveis.map((s) => (
              <button
                ...
                className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${subItemId && formatarParaUrl(subItemId) === formatarParaUrl(s) ? 'bg-[#0284C7]/10 text-[#0284C7] font-semibold' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}
              >
          ...
          {subItensDisponiveis.length > 12 && (
            <button
              ...
              className="text-left text-[11px] font-bold text-[#0284C7] hover:text-[#0369A1] transition-colors block pt-1"
            >
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3 shadow-[0_2px_8px_rgba(0,0,0,0.01)]">
          ...
          <div className="flex flex-col gap-1">
            <button
              ...
              className={`w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${!filtroTipoItem ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'}`}
            >
              Todos os tipos
            </button>
            {metadadosFiltros.tipos
              ...
              .map(tipo => (
                <button
                  ...
                  className={`w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition-colors truncate ${filtroTipoItem === tipo ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'}`}
                >
                  {tipo}
                </button>
              ))}
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3 shadow-[0_2px_8px_rgba(0,0,0,0.01)]">
          ...
          <div className="flex flex-col gap-1">
            <button
              ...
              className={`w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${!filtroEstado ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'}`}
            >
              Qualquer lugar
            </button>
            {metadadosFiltros.estados.map(estado => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition-colors truncate ${filtroEstado.toLowerCase() === estado.toLowerCase() ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'}`}
                >
                  {nomeExibicao}
                </button>
              );
