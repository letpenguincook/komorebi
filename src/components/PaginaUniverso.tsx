...
export const PaginaUniverso: React.FC<PaginaUniversoProps> = ({
  ...
  return (
    <div className="w-full min-h-screen bg-[#F8FAFC] relative">
      ...
      <header className="bg-transparent px-6 pt-8 pb-4">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono font-bold text-slate-400 cursor-pointer hover:text-slate-600" onClick={() => navigate('/')}>home</span>
            <span className="text-slate-300 text-xs">/</span>
            <h1 className="text-xl font-black text-slate-900 tracking-tight lowercase">
              k/
              <span className={`font-extrabold cursor-pointer ${(!grupoId && !subItemId) ? 'text-[#0284C7]' : 'text-slate-800'}`} onClick={() => navegarLimpo(`/${formatarParaUrl(universoId)}`)}>
                {formatarParaUrl(universoId)}
              </span>
              {grupoId && (
                <>
                  <span className="text-slate-300 font-normal">/</span>
                  <span className={`font-bold cursor-pointer ${!subItemId ? 'text-[#0284C7]' : 'text-slate-500'}`} onClick={() => navegarLimpo(`/${formatarParaUrl(universoId)}/${formatarParaUrl(grupoId)}`)}>
      ...
      <div className={`
        fixed top-0 left-0 bottom-0 w-80 max-w-[85vw] bg-[#F8FAFC] z-50 p-6 
      ...
      `}>
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-xs font-black uppercase tracking-wider text-slate-700 font-mono flex items-center gap-2">
            <i className="fa-solid fa-sliders text-slate-400" /> Filtros Avançados
          </h2>
          <button
            onClick={() => setIsMobileFiltersOpen(false)}
            className="w-7 h-7 flex items-center justify-center rounded-xl bg-white border border-slate-200 text-slate-400 text-sm font-bold hover:text-slate-600 shadow-sm"
          >
            ×
          </button>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <main className="md:col-span-9 space-y-4">
          <div className="bg-[#F8FAFC]/95 backdrop-blur-sm pb-2 pt-2">
            <BarraBuscaOrdenacao
          ...
          <button
            ...
            className="w-full md:hidden flex items-center justify-center gap-2 py-3 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-700 shadow-sm hover:bg-slate-50 transition"
          >
          ...
          {produtosPaginados.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center text-center py-20 border border-dashed border-slate-200 rounded-3xl bg-white p-6">              <i className="fa-solid fa-magnifying-glass-blur text-2xl text-slate-300 mb-3" />
              <h3 className="text-xs font-semibold text-slate-700">Nenhum mimo encontrado aqui</h3>

              {sugestaoRedirecionamento ? (
                <div className="mt-4 p-4 bg-[#0284C7]/5 border border-[#0284C7]/20 rounded-2xl max-w-sm">
                  <p className="text-[11px] text-slate-600 leading-relaxed">
                    {sugestaoRedirecionamento.tipo === 'grupo' ? (
                      <>Não encontramos isso em <span className="font-semibold">{formatarParaExibicao(grupoId || '')}</span>, mas temos mimos correspondentes na aba da obra <span className="font-bold text-[#0F172A]">{formatarParaExibicao(sugestaoRedirecionamento.nome)}</span>!</>
                    ) : (
                      <>Não encontramos isso em {formatarParaExibicao(universoId)}, mas achamos itens correspondentes lá no universo <span className="font-bold text-[#0F172A]">k/{sugestaoRedirecionamento.nome}</span>!</>
                    )}
                  </p>
                  <button
                ...
                <p className="text-[11px] text-slate-400 mt-1 max-w-xs">Não encontramos nenhum produto com o termo "{buscaInterna}".</p>
              )}
            </div>
          )
          }
